...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - args:
          - "/opt/edp/${service.serviceName}/conf/bootstrap-masterserver.sh"
        image: "${dockerImage}"
        ...
        readinessProbe:
          exec:
            ...
            - "/bin/bash"
            - "-c"
            - "curl --fail --connect-timeout 15 --max-time 15 \"http://`hostname`:${conf['master.server.port']}/actuator/health/liveness\"\
            \n"
          failureThreshold: 3
