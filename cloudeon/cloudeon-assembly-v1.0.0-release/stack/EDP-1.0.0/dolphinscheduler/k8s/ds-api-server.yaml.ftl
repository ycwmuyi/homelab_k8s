...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - args:
          - "/opt/edp/${service.serviceName}/conf/bootstrap-apiserver.sh"
        image: "${dockerImage}"
        ...
        readinessProbe:
          exec:
            ...
            - "/bin/bash"
            - "-c"
            - "curl --fail --connect-timeout 15 --max-time 15 \"http://`hostname`:${conf['api.server.port']}/dolphinscheduler/actuator/health/liveness\"\
            \n"
          failureThreshold: 3
